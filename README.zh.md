...
> Note: This document is a translation of the English version, and the content may not be updated in a timely manner. If there is any inconsistency, please refer to the English version. Last updated based on Sinatra v3.0.2.

> 注：本文档是英文版的翻译，内容更新有可能不及时。如有不一致的地方，请以英文版为准。最后一次更新基于 Sinatra v3.0.2。

Sinatra是一个基于Ruby的[领域特定语言(DSL)](https://en.wikipedia.org/wiki/Domain-specific_language)，致力于轻松、快速地创建网络应用。
...
你需要重启你的服务，才能看到代码更改生效。

每次代码更改都需要重启服务，或者使用帮助重启服务的工具，如 [rerun](https://github.com/alexch/rerun) 或者
[rack-unreloader](https://github.com/jeremyevans/rack-unreloader)。

这里推荐运行 `gem install puma` ，这样 Sinatra 会优先选择 puma 作为服务器。
...
在 Sinatra 中，一个路由两部分组成：HTTP方法 和 URL匹配模式。每个路由都有一个要执行的代码块：
