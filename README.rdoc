...
`verb` condition can be used to define filters for routes with the same URL
pattern but different HTTP method:

  before '/index', :verb => /post/i do
  end

Both String and Regexp patterns can be used with the `verb` condition.
